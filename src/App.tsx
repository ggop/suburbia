...
export default function App() {
  ...
  const handleMapClickDisabled = useCallback(() => {
    setErrorMessage('Map clicking is disabled. Type an adjacent suburb name in the box to navigate.');
    sounds.playError();
  }, []);
  ...
  const handleMoveToSuburb = useCallback(
    (nextSuburbId: string) => {
      ...
      // Already in current position
      if (nextSuburbId === currentId) {
        setErrorMessage(`You are currently in ${currentSuburb.name}. Choose an adjacent suburb.`);
        sounds.playError();
        setConsecutiveErrors((prev) => prev + 1);
        return;
      }
      ...
      if (gameState.path.includes(nextSuburbId)) {
        setErrorMessage(`You have already visited ${nextSuburb.name}! Pick an unvisited adjacent suburb.`);
        sounds.playError();
        setConsecutiveErrors((prev) => prev + 1);
        return;
      }
      ...
      if (!isAdjacent) {
        setErrorMessage(
          `${nextSuburb.name} is not directly adjacent to ${currentSuburb.name}. Choose a bordering suburb!`
        );
        sounds.playError();
        setConsecutiveErrors((prev) => prev + 1);
        return;
      ...
      // Valid tactical move! Reset consecutive errors so hint clears for the next step
      sounds.playStep();
      ...
      if (newTurnsUsed >= gameState.maxTurns) {
        setGameState((prev) => ({
          ...prev,
          path: newPath,
          turnsUsed: newTurnsUsed,
  ...
  const handleUndoLastMove = useCallback(() => {
    if (gameState.path.length <= 1 || gameState.status !== 'playing') return;
    setGameState((prev) => ({
      ...prev,
    ...
    setErrorMessage(null);
    setConsecutiveErrors(0);
  }, [gameState.path.length, gameState.status]);
