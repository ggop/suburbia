...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header
      id="game-header"
      className="h-16 border-b border-neutral-200 bg-white flex items-center justify-between px-4 sm:px-6 z-30 shadow-xs shrink-0 select-none text-neutral-900"
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] text-neutral-500 truncate max-w-xs sm:max-w-md lg:hidden">
            ...
            <span className="text-blue-600 font-semibold">{targetSuburb?.name || 'Target'}</span> (≤ 10 turns)
          </p>
        </div>
      </div>

      {/* Center Route Status (Matching Design HTML) */}
      ...
      <div className="flex items-center gap-2">
        ...
        {!isGameOver && onGiveUp && (
          <button
            ...
            className="px-3 py-1.5 rounded-lg text-xs font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 flex items-center gap-1.5 transition-colors cursor-pointer"
          >
        ...
        <button
          ...
          onClick={onNewGame}
          title="Start a new challenge with random suburbs"
          className="px-3.5 py-1.5 rounded-lg text-xs font-bold bg-black hover:bg-neutral-800 text-white transition-all shadow-xs flex items-center gap-1.5 active:scale-95"
        >
